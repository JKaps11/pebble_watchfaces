// teebox — standing on the tee, the hole laid out in front of you.
//
// Axes: time-display=digital, composition=split, complications=1-2,
//       hue=multi, type=custom, polarity=light-on-dark
//
// Every scene so far has floated somewhere over the fairway with nothing
// underfoot. This puts the viewer on the tee deck: a level platform mown lighter
// than anything beyond it, markers set, a ball waiting. It also earns the split
// that band had to assert — the black does not start at an arbitrary row, it
// starts where the deck stops, so the straight edge is the front of the tee.
...
static void prv_load(Window *window) {
  ...
  s_date_layer = studio_text_layer(
      window_layer, GRect(12, 154, 116, 22),
      fonts_get_system_font(FONT_KEY_GOTHIC_18_BOLD), GColorWhite,
      GTextAlignmentLeft);

  s_battery_layer = layer_create(GRect(152, 158, 36, 14));
...
STUDIO_VARIANT("teebox", prv_load, prv_unload, prv_tick)
