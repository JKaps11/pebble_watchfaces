// band — the foreground as a ruled black bar. The one to beat.
//
// Axes: time-display=digital, composition=split, complications=1-2,
//       hue=multi, type=custom, polarity=light-on-dark
//
// Batch 2's sunrise, unchanged except for the richer hole behind it, so that the
// rest of this Sweep has a fixed thing to be compared against. The complaint it
// exists to answer is that a hard rectangle cut across a landscape reads as two
// pictures stacked rather than one — the scene stops, and something unrelated
// starts. Everything else here is a different way of ending the scene.
...
STUDIO_VARIANT("band", prv_load, prv_unload, prv_tick)
